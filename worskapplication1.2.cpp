...
int minimal_pixel(int **array,int n,int k)              
{
	if(k==2)
	return 1;
	int i,j,prev,l,ans=n,temp,m,position,diff=1;
	for(l=0;l<n;l++)
	{
	tree *mango;
	mango = new tree();
	...
	for(i=0;i<n;i++)
	{
		...
		if(prev<mango->return_siblings())         // at least one node with both children at previous level 
			{
			 ...
			 if(i!=n-1)
			 {
			 	if(mango->return_siblings()-prev>=diff)
			 	{
			 		diff = mango->return_siblings()-prev;
			 		position =i+1;
				 }
			 }
			}                  
		...
		if(mango->return_siblings()==k)          //all numbers distinguished
		   {
		   	if(ans>mango->return_index())
		   	ans = mango->return_index();
		   	break;
		  }   
	}
	if(ans==2)
	...
	for(j=0;j<k;j++)                          // shift positions to produce other combinations
	{
		temp = array[j][position];
		for(m=l+1;m<=position;m++)
